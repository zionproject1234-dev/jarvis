...
import { Terminal, Send, Cpu, Mail, CheckSquare, Zap, X, LogOut, Mic, MicOff, CloudSun, Palette, Minus, Square, Sun, Moon } from 'lucide-react';
...
function App() {
  ...
  useEffect(() => {
    ...
    const {
      ...
      if (session) {
        ...
        // Greet user on login
        setTimeout(() => {
          speak(`Welcome back, sir. Google Cloud Link established. All mission directives are online.`);
        }, 1000);
      }
    });
  ...
  const toggleTask = async (task) => {
    ...
    try {
      ...
      console.warn('Supabase toggle sync failed, state already updated locally:', e);
    }
  };
  ...
  const addTask = async (title, priority = 'Medium') => {
    ...
    const newTask = { id: tempId, title: title.trim(), completed: false, priority };
    ...
    try {
      const { data, error } = await supabase
        .from('tasks')
        .insert([{ title: title.trim(), user_id: session.user.id, completed: false }])
        .select();

      if (!error && data && data[0]) {
        setTasks(prev => prev.map(t => t.id === tempId ? { ...data[0], priority } : t));
        await supabase.from('tasks').update({ priority }).eq('id', data[0].id).catch(() => { });
      }
    } catch (e) {
      console.warn('Supabase sync failed, task stays local:', e);
    }
  };
...
Return NOTHING else — no explanation, no punctuation, just the 11-character ID.`;
        ...
        response = "Comms interface engaged. I've initialized the drafting protocols for your approval.";
        setActivePanel('email');

        // Auto-drafting logic
        if (lowerInput.length > 10) {
          const draftPrompt = `The user wants to send an email: "${textInput}". 
          Draft a professional but JARVIS-like email. Output only valid JSON with fields: recipient, subject, body.`;
          try {
            const draftResult = await model.generateContent(draftPrompt);
            const draftText = draftResult.response.text();
            const draftJson = JSON.parse(draftText.match(/\{.*\}/s)[0]);
            setEmailData({
              to: draftJson.recipient || 'pepper.potts@starkindustries.com',
              subject: draftJson.subject || 'Automated Transmission',
              body: draftJson.body || ''
            });
          } catch (e) {
      ...
      // Only call Gemini if no local action handled the request
      if (!localActionTriggered) {
        try {
          const prompt = `You are J.A.R.V.I.S., the highly sophisticated AI assistant to Tony Stark. 
          Keep responses concise, professional, and helpful. 
          Current environment: Web app with system access.
          The user says: "${textInput}". 
        ...
        <div style={{ position: 'absolute', top: 120, right: 20, width: 200 }}>
          ...
          <div className="hud-widget">
            ...
            <div style={{ display: 'flex', gap: '5px' }}>
              ...
              <span style={{ fontSize: '0.8rem' }}>GOOGLE CLOUD SYNCED</span>
            </div>
          </div>
        </div>
          ...
          {activePanel === 'tasks' && (
            ...
            >
              ...
              <div className="tasks-list" style={{ flex: 1, overflowY: 'auto' }}>
                <h3 style={{ fontSize: '0.8rem', color: 'var(--neon-cyan)', marginBottom: '10px' }}>UPCOMING SCHEDULE (G-SYNC)</h3>
                {events.map(event => (
                  <div key={event.id} style={{
                    display: 'flex', justifyContent: 'space-between',
                    padding: '10px', borderLeft: '3px solid var(--neon-cyan)',
                    background: 'rgba(0,240,255,0.05)', marginBottom: '10px'
                  }}>
                    <div>
                      <div style={{ fontWeight: 'bold', fontSize: '0.9rem' }}>{event.title}</div>
                      <div style={{ fontSize: '0.7rem', color: 'var(--text-dim)' }}>{event.loc}</div>
                    </div>
                    <div style={{ fontSize: '0.8rem', color: 'var(--neon-cyan)' }}>{event.time}</div>
                  </div>
                ))}
                ...
                <h3 style={{ fontSize: '0.8rem', color: 'var(--neon-cyan)', marginTop: '20px', marginBottom: '10px' }}>ACTIVE CHRONOMETER REMINDERS</h3>
                {reminders.filter(r => r.active).map(reminder => (
                  <div key={reminder.id} style={{
                    padding: '10px', borderLeft: '3px solid #f00',
                    background: 'rgba(255,0,0,0.05)', marginBottom: '10px',
                    fontSize: '0.9rem'
                  }}>
                    <div style={{ fontWeight: 'bold' }}>{reminder.text}</div>
                    <div style={{ fontSize: '0.75rem', color: 'var(--text-dim)' }}>
                      DUE: {reminder.dueDate.toLocaleTimeString()}
                    </div>
                  </div>
                ))}
